...
def has_winner(board_state):
    ...
    for x in range(3):
        if _has_3_in_a_line(board_state[x]):
            return board_state[x][0],str(x*3)+str(x*3+1)+str(x*3+2)
    # check columns
    for y in range(3):
        if _has_3_in_a_line([i[y] for i in board_state]):
            return board_state[0][y],str(y)+str(y+3)+str(y+6)

    # check diagonals
    if _has_3_in_a_line([board_state[i][i] for i in range(3)]):
        return board_state[0][0],str(0)+str(4)+str(8)
    if _has_3_in_a_line([board_state[2 - i][i] for i in range(3)]):
        return board_state[0][2],str(2)+str(4)+str(6)
...
if __name__ == '__main__':
    ...
    play_game(human_player, random_player, log=True)
